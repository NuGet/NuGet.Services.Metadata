...
{
    public class ExcludeIdDataClient : IExcludeIdDataClient
    {
        ...
        public async Task<ResultAndAccessCondition<ExcludeIdData>> ReadLatestIndexedAsync()
        {
            var stopwatch = Stopwatch.StartNew();
            var blobName = GetLatestIndexedBlobName();
            var blobReference = Container.GetBlobReference(blobName);

            _logger.LogInformation("Reading the latest exclude Id list from {BlobName}.", blobName);

            ExcludeIdData data = null;
            ...
            var output = new ResultAndAccessCondition<ExcludeIdData>(
                data,
                accessCondition);

            stopwatch.Stop();
            _telemetryService.TrackReadLatestIndexedExcludeIdList(output.Result.Ids.Count(), stopwatch.Elapsed);

            return output;
        }

        public async Task ReplaceLatestIndexedAsync(
            ExcludeIdData newData,
            IAccessCondition accessCondition)
        {
            using (_telemetryService.TrackReplaceLatestIndexedOwners(newData.Ids.Count()))
            {
                var blobName = GetLatestIndexedBlobName();
        ...
        private static ExcludeIdData ReadStream(Stream stream)
        {
            ...
            using (var jsonReader = new JsonTextReader(textReader))
            {
                ...
                {
                    var excludeListPropertyName = (string)jsonReader.Value;

                    Guard.Assert(jsonReader.Read(), "There should be a token after the property name.");
                    Guard.Assert(jsonReader.TokenType == JsonToken.StartArray, "The token after the property name should be the start of an array object.");

                    excludeList = Serializer.Deserialize<IEnumerable<string>>(jsonReader);
            ...
            return new ExcludeIdData { Ids = excludeList };
        }

        private string GetLatestIndexedBlobName()
        {
            return $"{_options.Value.NormalizeStoragePath()}ExcludeIdList/ExcludeIdList.v1.json";
        }
    }
}
