...
{
    /// TODO: This is hacked copy of <see cref="StorageLoader" />
    public class BlobLoader : ILoader
    {
        ...
        public BlobLoader(IndexingConfiguration config, FrameworkLogger logger)
        {
            _logger = logger;
            Reload(config);
        }
        ...
        public JsonReader GetReader(string blobName)
        {
            try
            {
                ...
                return new JsonTextReader(new StreamReader(blob.OpenReadAsync().GetAwaiter().GetResult()));
            }
            catch (Exception e)
            {
